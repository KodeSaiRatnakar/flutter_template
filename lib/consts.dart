...
//TODO! Move these queries to dbquery.json
String topicListQuery({
  String? parentTopicUri,
  int? limit = 30,
}) {
  const sqlSticky = '0 AS sticky';
  var where = '';

  if (parentTopicUri != null) {
    where = "WHERE parent_topic_uri = '$parentTopicUri' "
        "OR row_topic_uri = '$parentTopicUri'";
  } else {
  ...
  var query = """
      SELECT
        ...
        CASE WHEN MAX(comment.added) IS NULL 
        THEN topic.added ELSE MAX(comment.added) END as last_action,
        topic.*,
  ...
  if (parentTopicUri == null) {
    query += """
      ...
      SELECT
        ...
        CASE WHEN MAX(topic_sub.added) > MAX(comment.added) OR MAX(comment.added) IS NULL 
        THEN MAX(topic_sub.added) ELSE MAX(comment.added) END as last_action,
        topic.*,
        topic_creator_user.value AS topic_creator_user_name,
        topic_creator_content.directory AS topic_creator_address,
        topic.topic_id || '_' || topic_creator_content.directory AS row_topic_uri,
